...
SRC_IMG=${1:-/mnt/c/Work/r36sx_disasm/local_artifacts/r36sx_debian12_mipsel_bootonly.img}
OUT_IMG=${2:-/mnt/c/Work/r36sx_disasm/local_artifacts/r36sx_debian12_mipsel_mtdfat.img}
...
required_sources=(
    "$WORKSPACE/disk_image/cubegm/dtb.dts"
    ...
    "$WORKSPACE/disk_image/Movie/sample.mp4"
    "$WORKSPACE/disk_image/Ebook/Little women.txt"
    "$WORKSPACE/disk_image/Music/Westlife - En Ti Deje Mi Amor (Spanish Version of 'I Lay My Love On You' Single Remix).mp3"
    "$WORKSPACE/disasm/mtd/mtd2_eromfs_extracted/popup.bmp.gz"
    "$WORKSPACE/disasm/mtd/mtd2_eromfs_extracted/batterylow.bmp.gz"
)
...
SRC_LOOP=
OUT_LOOP=
SRC_ROOT=/mnt/r36sx-mtdfat-src-root
OUT_BOOT=/mnt/r36sx-mtdfat-out-boot
OUT_ROOT=/mnt/r36sx-mtdfat-out-root
DTB_WORK=
...
mkdir -p "$OUT_BOOT/cubegm" "$OUT_BOOT/Movie" "$OUT_BOOT/Ebook" "$OUT_BOOT/Music"
...
rsync -rt "$WORKSPACE/disk_image/Movie/sample.mp4" "$OUT_BOOT/Movie/"
rsync -rt "$WORKSPACE/disk_image/Ebook/Little women.txt" "$OUT_BOOT/Ebook/"
rsync -rt "$WORKSPACE/disk_image/Music/Westlife - En Ti Deje Mi Amor (Spanish Version of 'I Lay My Love On You' Single Remix).mp3" "$OUT_BOOT/Music/"

# These names are embedded in MTD2 EROMFS. Keep them at FAT root so a simple
# name lookup can find them if the firmware probes for the resources there.
...
echo "Created MTD-referenced FAT Debian image:"
ls -lh "$OUT_IMG"
