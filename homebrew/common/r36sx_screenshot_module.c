...
static void r36sx_module_put_le16(uint8_t *dst, uint16_t value)
{
    dst[0] = (uint8_t)(value & 0xffu);
    dst[1] = (uint8_t)((value >> 8) & 0xffu);
}

static void r36sx_module_put_le32(uint8_t *dst, uint32_t value)
{
    dst[0] = (uint8_t)(value & 0xffu);
    dst[1] = (uint8_t)((value >> 8) & 0xffu);
...
static void r36sx_module_rgb565_to_bgr24(uint8_t *dst,
                                         const uint16_t *src,
                                         size_t count)
{
    for (size_t i = 0; i < count; i++) {
        ...
        dst[i * 3u + 0u] = r36sx_png_rgb565_to_b8(color);
        dst[i * 3u + 1u] = r36sx_png_rgb565_to_g8(color);
        dst[i * 3u + 2u] = r36sx_png_rgb565_to_r8(color);
    }
}

static int r36sx_module_write_bmp24(const char *path,
                                    ...
                                    int height)
{
    ...
    header[0] = 'B';
    header[1] = 'M';
    r36sx_module_put_le32(&header[2],
                          (uint32_t)(sizeof(header) + pixel_bytes));
    r36sx_module_put_le32(&header[10], (uint32_t)sizeof(header));
    r36sx_module_put_le32(&header[14], 40u);
    r36sx_module_put_le32(&header[18], (uint32_t)width);
    r36sx_module_put_le32(&header[22], (uint32_t)height);
    r36sx_module_put_le16(&header[26], 1u);
    r36sx_module_put_le16(&header[28], 24u);
    r36sx_module_put_le32(&header[34], (uint32_t)pixel_bytes);
    ...
    for (int y = height - 1; ok && y >= 0; y--) {
        ...
        r36sx_module_rgb565_to_bgr24(row, src, (size_t)width);
...
static int r36sx_module_write_png24(const char *path,
                                    const uint16_t *pixels,
                                    int width,
                                    int height)
{
...
int r36sx_screenshot_write_rgb565(const char *path,
                                  ...
                                  int format)
{
    if (format == R36SX_SCREENSHOT_FORMAT_BMP) {
        return r36sx_module_write_bmp24(path, pixels, width, height);
    }
    if (format == R36SX_SCREENSHOT_FORMAT_PNG) {
        return r36sx_module_write_png24(path, pixels, width, height);
    }
    return -1;
}
