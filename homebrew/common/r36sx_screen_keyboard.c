...
#define R36SX_OSK_LOCK_LED_RADIUS 4
#define R36SX_OSK_LOCK_LED_OUTER_RADIUS 5
...
static void draw_lock_led(uint16_t *frame, int width, int height,
                          int stride, int cx, int cy, int enabled)
{
    const int radius = R36SX_OSK_LOCK_LED_RADIUS;
    const int outer_radius = R36SX_OSK_LOCK_LED_OUTER_RADIUS;
...
static int keyboard_header_h_for_state(
    const struct r36sx_screen_keyboard *keyboard)
{
    return keyboard && keyboard->expanded ? R36SX_OSK_HEADER_H : 0;
}

static int keyboard_view_h_for_panel(int panel_h, int header_h)
{
    int view_h = panel_h - 2 - header_h - 2 * R36SX_OSK_INNER_PAD;
    ...
    const struct r36sx_screen_keyboard *keyboard)
{
    return keyboard && keyboard->expanded ?
        keyboard_view_h_for_panel(keyboard_fit_panel_h(),
                                  R36SX_OSK_HEADER_H) :
        keyboard_view_h_for_panel(R36SX_SCREEN_KEYBOARD_PANEL_H, 0);
}
...
static int row_tail_extra_col(int count)
{
    return count - 1;
}

static int bottom_row_tail_extra_for_col(int col, int tail_extra)
{
    int space_extra;
    int right_alt_extra;
    int remaining;
    ...
    remaining = tail_extra - space_extra;
    right_alt_extra = remaining > 2 ? 2 : remaining;
    remaining -= right_alt_extra;

    if (col == 3) {
        return space_extra;
    }
    if (col == 4) {
        return right_alt_extra;
    }
    if (col == 6) {
        return remaining;
    }
...
static int row_tail_extra_for_col(int row,
                                  int col,
                                  int count,
                                  int tail_extra)
{
    if (row == 5 && count > 6) {
        return bottom_row_tail_extra_for_col(col, tail_extra);
    }
    return col == row_tail_extra_col(count) ? tail_extra : 0;
}
                             ...
                             int force_shift)
{
    ...
    if (!keyboard || repeat_button == 0 || keycode == 0 ||
        ...
        keycode == R36SX_SCREEN_KEY_SCROLL) {
        return;
    }
                     ...
                     int pressed)
{
                ...
                R36SX_OSK_KEY_H, border);
    {
        ...
        if (key->keycode == R36SX_SCREEN_KEY_CAPITAL ||
            key->keycode == R36SX_SCREEN_KEY_SCROLL) {
            int led_outer = R36SX_OSK_LOCK_LED_OUTER_RADIUS;
            int led_pad = key->keycode == R36SX_SCREEN_KEY_SCROLL ? 4 : 5;
            int led_x = x + key_w - led_outer - led_pad;
            int led_left = led_x - led_outer;
            int label_w = led_left - x;
            int led_on = key->keycode == R36SX_SCREEN_KEY_CAPITAL ?
                keyboard->caps_lock != 0 : keyboard->scroll_lock != 0;
            if (label_w < 1) {
                label_w = 1;
            }
            ...
            draw_lock_led(frame, width, height, stride,
                          led_x,
                          y + R36SX_OSK_KEY_H / 2,
                          led_on);
        }
    ...
    int stride_pixels)
{
    ...
    const int header_h = keyboard_header_h_for_state(keyboard);
    const int view_y = panel_y + 1 + header_h + R36SX_OSK_INNER_PAD;
    const int view_h = keyboard_view_h_for_panel(panel_h, header_h);
    ...
    stroke_rect(frame, width, height, stride_pixels, panel_x, panel_y, panel_w,
                panel_h, border);
    if (header_h > 0) {
        fill_rect(frame, width, height, stride_pixels, panel_x + 1,
    ...
    for (size_t row = 0; row < (size_t)keyboard_row_count(); row++) {
        ...
        for (int col = 0; col < count; col++) {
            ...
            key_w += row_tail_extra_for_col((int)row, col, count,
                                            tail_extra);
