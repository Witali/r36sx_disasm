...
passed to `MIPS_NATIVE/common/screenshot.so`; if that module is missing, BMP
falls back to the built-in writer while PNG saves fail cleanly.
...
legacy R36SX test386 POST port `190h`.  The rebuilt `test386.bin` writes POST
...
set to the selected letter or higher.  To map the drive automatically, add the
