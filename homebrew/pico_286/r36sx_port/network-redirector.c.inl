...
{
#if defined(R36SX_PICO286_HOST_DRIVE_CONFIG)
    const char *path = r36sx_pico286_host_drive_path();
    return path && path[0] ? path : HOST_BASE_DIR;
#else
    return HOST_BASE_DIR;
#endif
}
...
static void build_host_path(char *dest, size_t dest_size, const char *tail) {
    ...
    if (tail[0] == '\0') {
        ...
        snprintf(dest, dest_size, "%s%s", base, tail);
    } else {
        snprintf(dest, dest_size, "%s%c%s", base, sep, tail);
    }
    dest[dest_size - 1] = '\0';
#ifndef WIN32
    for (char *p = dest; *p; p++) {
...
static void get_full_path(char *dest, const char *guest_path) {
    ...
    if (guest_path[1] == ':') {
        ...
        build_host_path(dest, 256, path_part);
    } else if (guest_path[0] == '\\') {
        // Root-relative path (e.g., "\subdir\file.txt")
        build_host_path(dest, 256, guest_path);
    } else {
        // Relative path (e.g., "file.txt" or "subdir\file.txt")
        if (strlen(current_remote_dir) > 0) {
            snprintf(relative_tail, sizeof(relative_tail), "%s\\%s",
                     current_remote_dir, guest_path);
        } else {
            snprintf(relative_tail, sizeof(relative_tail), "%s", guest_path);
        }
        relative_tail[sizeof(relative_tail) - 1] = '\0';
        build_host_path(dest, 256, relative_tail);
    }
    dest[255] = '\0';
#ifndef WIN32
    char *p = strstr(dest, "????????.???");
