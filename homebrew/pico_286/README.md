...
`0F 22`.  VM86 `LOCK`, `PUSHF`, `POPF`, `INT imm8`, and `IRET/IRETD` now trap
with `#GP(0)` when `IOPL < 3`, which is the entry point a real monitor would
use.  Linear physical memory above 1 MB is backed by the same XMS buffer, so
simple flat descriptors can access the configured extended RAM.  Paging, full
VM86 monitor behavior, and DOS extender services such as DPMI/VCPI are not
complete yet, so keep `cpu_mode=real` for normal DOS use.
