...
- `r36sx_minifb.c` implements the small upstream `MiniFB` API through
  ...
  output frame and skip DOS-frame composition.  Small overlays save their
  covered rectangles, draw into `SCREEN`, and restore those rectangles at the
  next `mfb_update()` unless the DOS renderer already produced a dirty frame.
  The expensive DOS-video renderer runs only after visible emulated video state
  changes.  `[video] keyboard_mode`
...
- `r36sx_linux_audio.c` implements upstream `linux-audio.h` through
  ...
  frame counts and sizes packets from elapsed time.
...
- `r36sx_minifb.c` also draws a blinking red disk activity indicator in the
  ...
  direct-present mode it uses the delayed-restore small-overlay path, while
  plain DOS frames still go straight from `SCREEN` to `driver.so`.  The app
  statistics overlay caches its pixel-font block between one-second statistics
  refreshes.
...
interrupt/trap gates, 16-bit and 32-bit call gates, call-gate privilege
transitions with TSS-backed stack switching, hardware task switches through
task gates/TSS descriptors, initial VM86 task entry and VM86 interrupt/IRETD
...
`0F 22` in native protected mode.  VM86 `LOCK`, `PUSHF`, `POPF`, `INT imm8`,
and `IRET/IRETD` now trap with `#GP(0)` when `IOPL < 3`, which is the entry
...
host loop now runs one budgeted emulation pass per frame: at 60 Hz the first
10 ms are reserved for small `exec86()` quanta of 100 instructions and audio
sample generation, while virtual PIT/audio/cursor time is stretched across the
full 16.7 ms frame.  The remaining frame time is reserved for rendering,
overlays, buffer handoff, and sleeping when there is spare time.  If the
emulation budget expires before the configured `cpu_mhz` instruction quota is
complete, the frame still advances timers for the full display period.
...
`screenshot_format=png` and `screenshot_format=bmp` are passed to the optional
`/mnt/sdcard/MIPS_NATIVE/common/screenshot.so` module.  That module contains
the zlib PNG writer and the BMP writer, so the main `pico_286` executable can
start without linking zlib directly.  If the module is missing, BMP still uses
the built-in fallback writer; PNG saves fail cleanly.
...
redirector does not permanently consume conventional memory.
