...
- `r36sx_linux_audio.c` implements upstream `linux-audio.h` through
  ...
  initialization.  The sound path double-buffers completed blocks, sizes them
  from `target_fps`, and applies a short de-click ramp at block boundaries.
...
The same target also sets the audio block size sent to `driver.so`; the default
60 Hz produces about 735 stereo frames per block at 44.1 kHz.
