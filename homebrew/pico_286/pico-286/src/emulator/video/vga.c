...
static const uint32_t vga_default_palette[256] = {
...
typedef struct {
    ...
    uint8_t sequencer[5];
    uint8_t crtc[25];
    uint8_t graphics_controller[9];
    uint8_t attribute_controller[21];
} vga_standard_regs_t;

static const vga_standard_regs_t vga_mode_80x25_text = {
    0x67,
...
static const vga_standard_regs_t vga_mode_320x200_16 = {
    0x63,
...
static const vga_standard_regs_t vga_mode_640x200_16 = {
    0x63,
...
static const vga_standard_regs_t vga_mode_640x350_16 = {
    0xA3,
    {0x00, 0x01, 0x0F, 0x00, 0x06},
    {0x5F, 0x4F, 0x50, 0x82, 0x54, 0x80, 0xBF, 0x1F,
...
static const vga_standard_regs_t vga_mode_640x480_2 = {
    0xE3,
    {0x00, 0x01, 0x0F, 0x00, 0x06},
...
static const vga_standard_regs_t vga_mode_640x480_16 = {
    0xE3,
    {0x00, 0x01, 0x0F, 0x00, 0x06},
    {0x5F, 0x4F, 0x50, 0x82, 0x54, 0x80, 0x0B, 0x3E,
     0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static const vga_standard_regs_t vga_mode_320x200_256 = {
    0x63,
...
static void vga_apply_standard_regs(const vga_standard_regs_t *regs)
{
    memset(&vga, 0, sizeof(vga));
    memcpy(vga.sequencer, regs->sequencer, sizeof(regs->sequencer));
    memcpy(vga.graphics_controller, regs->graphics_controller,
           sizeof(regs->graphics_controller));
    memset(crt_controller, 0, sizeof(crt_controller));
    memcpy(crt_controller, regs->crtc, sizeof(regs->crtc));
    memset(attribute_controller, 0, sizeof(attribute_controller));
    memcpy(attribute_controller, regs->attribute_controller,
           sizeof(regs->attribute_controller));

    misc_output_register = regs->misc;
    sequencer_register = 0;
    graphics_control_register = 0;
    crt_controller_idx = 0;
    attribute_data_mode = 0;
    vga_register = 0;
    vga_latch32 = 0;
    vga_plane_offset = 0;
    vram_offset = ((uint32_t)crt_controller[0x0C] << 8) |
                  (uint32_t)crt_controller[0x0D];
    vga_svga_disable();
    vga_update_seq_cache();
    vga_update_gc_cache();
    cga_blinking = (attribute_controller[0x10] & 0x20u) ? 0x7F : 0xFF;
}
