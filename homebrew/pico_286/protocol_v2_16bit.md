...
Этот документ описывает план апгрейда HOSTRPC stream protocol с текущего
...
- Сохранить совместимость с текущим v1 8-bit stream protocol.
...
v1 остается обязательным базовым режимом. Любой гость должен уметь работать
через текущие 8-битные кадры:

| Режим | Порт | I/O width | Payload bits | Request address frames |
| --- | --- | --- | --- | --- |
| v1 | `E360h` | 8-bit | 7 | 5 |
| v2 | `E360h` | 16-bit | 15 | 3 |

Гость выбирает v2 только после успешного обнаружения capability. Если host не
поддерживает 16-битный режим, гость остается на v1.
...
ID `5` свободен после удаления старого `CALL`, поэтому его можно занять под
v1-safe capability query. Старый host, который не знает `PROTO_CAPS`, вернет
`PROTO_ERR` (`7Fh`) или не подтвердит команду ожидаемым образом; guest обязан
в этом случае использовать v1.

Первый capability word можно читать через v1, чтобы договориться о v2 без
предварительного использования 16-битного I/O:
...
Так как v1 frame несет только 7 бит ответа, capability bitmap можно вернуть
двумя v1 frames: первый ответ на `PROTO_CAPS` содержит bits `0..6`, следующий
...
1. Прочитать `E360h` и сохранить v1 sync bit.
2. Отправить `RESET`.
3. Отправить `PING`, проверить `RPC_VERSION >= 1`.
4. Отправить `PROTO_CAPS`.
5. Если ответ успешен и выставлен `RPC_CAP_STREAM16`, переключиться на v2.
6. Иначе продолжить работу через v1.
...
Командное пространство не меняется. Управляющие команды `RESET`, `PING`,
`CONTINUE`, `ABORT`, `END` и host commands `CMD_*` остаются теми же id, что в
v1. Отличается только ширина frame и количество полезных бит.
...
Request block layout можно оставить совместимым с v1:
...
Поле `version` можно оставить `1`, если меняется только транспортный frame
format. Если позже появятся новые поля request block, тогда нужно увеличить
именно request-block ABI version до `2`.
...
2. Оставить текущий byte handler как v1.
3. Добавить word handler для v2:
...
- `ABORT` должен сбрасывать незавершенную v2-передачу address frames.
- `RESET` должен сбрасывать и v1, и v2 stream state, а также закрывать stale
