...
2. Отправить `RESET`.
3. Отправить `PING`, проверить `RPC_VERSION >= 4`.
...
зарезервировано как общий protocol error marker. Детализированные transport
errors в stream16 должны соответствовать 8-bit режиму: mismatch состояния
протокола и слишком длинная fixed-size посылка должны возвращать отдельные
коды, а не смешиваться с обычными result codes.
...
Поле `version` остается `4`, если меняется только транспортный frame format.
