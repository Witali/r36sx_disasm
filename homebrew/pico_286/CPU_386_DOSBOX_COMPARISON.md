...
| Protected descriptor opcodes in VM86 | `ARPL`, `LAR`, `LSL`, and `0F 00` descriptor instructions ran in VM86 because VM86 still has `CR0.PE=1`. | Intel/DOSBox reject these protected-mode instructions in real-address and virtual-8086 mode with invalid opcode. | These opcodes now require native protected mode, so VM86 gets `#UD` instead of descriptor-table side effects. |
