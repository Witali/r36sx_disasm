# MTD1 Boot Chain Analysis

This file describes what was recovered from the Ghidra disassembly of
`hardware_info/mtd-dump/mtd1_boot.bin`, and where the evidence from the MTD code
ends.

## Short Conclusion

Ghidra shows an earlier boot chain rather than a direct load of
...
`cubegm/vmlinux.uImage` are not present in the current first/stage2 Ghidra
output.
...
The second profile is required because the bytes at file offset `0x3000` are
not meant to execute at `0xbfe03000`; they are copied into RAM and executed
through MIPS cached/uncached aliases.
...
`0xb884c000`. The first-stage Ghidra output has several `0xb884...` MMIO
regions classified as likely USB/MMC-side peripheral initialization, but the
exact SD read and FAT path parser are not proven in first/stage2.
